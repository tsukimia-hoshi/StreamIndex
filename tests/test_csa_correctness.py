...
def test_csa_forward_triton_matches_reference:
 ...
 q, kv, kv_compressed, q_idx, k_idx_compressed, weights, attn_sink,
 n_win=n_win, top_k=top_k, m=m, use_triton=True,
 )
 # Reference path
 o_ref = flash_csa_forward(
 q, kv, kv_compressed, q_idx, k_idx_compressed, weights, attn_sink,
 n_win=n_win, top_k=top_k, m=m, use_triton=False,
 )
...
def test_csa_forward_chunked_indexer_matches_unchunked:
 ...
 q, kv, kv_compressed, q_idx, k_idx_compressed, weights, attn_sink,
 n_win=n_win, top_k=top_k, m=m, use_triton=True, use_chunked_indexer=False,
 )
 o_chunked = flash_csa_forward(
 q, kv, kv_compressed, q_idx, k_idx_compressed, weights, attn_sink,
 n_win=n_win, top_k=top_k, m=m, use_triton=True, use_chunked_indexer=True,
 chunk_s=16, chunk_t=8,
 )
