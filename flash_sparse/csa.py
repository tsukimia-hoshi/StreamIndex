 ...
 *,
 ...
 block_last = (
 torch.arange(n_compressed, device=q.device) + 1
 ) * m - 1
 ...
 q_idx, k_idx_compressed, indexer_weights,
 top_k=top_k, causal_mask=causal,
 ...
 pad = torch.full(
 (B, S, top_k - top_idx_in_comp.shape[-1]), -1,
 dtype=top_idx_in_comp.dtype, device=q.device,
 )
 top_idx_in_comp = torch.cat([top_idx_in_comp, pad], dim=-1)

 win_idxs = _build_sliding_window_idxs(B, S, n_win, q.device)
 minus_one = torch.full_like(top_idx_in_comp, -1)
 comp_idxs_global = torch.where(
 top_idx_in_comp >= 0, top_idx_in_comp + S, minus_one
 )
 ...
 scores, n_win=n_win, top_k=top_k,
 seq_len=S, n_compressed=n_compressed, m=m, device=q.device,
 )
