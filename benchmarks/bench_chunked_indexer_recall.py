...
def _build_inputs(cfg: RecallCfg, seed: int = 2026):
 ...
 sigma = cfg.D_I ** -0.5
 q = torch.randn(cfg.B, cfg.S, cfg.H_I, cfg.D_I,
 generator=g, device="cuda", dtype=torch.bfloat16) * sigma
 k_idx = torch.randn(cfg.B, cfg.T, cfg.D_I,
 generator=g, device="cuda", dtype=torch.bfloat16) * sigma
 weights = torch.randn(cfg.B, cfg.S, cfg.H_I,
...
def main:
 ...
 gap = _tied_gap(scores, cfg.top_k)
 print(f" ground-truth tie gap: median={gap['median_gap']:.3e},"
 ...
 print(f" {'(chunk_s, chunk_t)':>20} {'mean':>8} {'min':>8} "
 f"{'%=1':>7} {'%<.99':>7}")
