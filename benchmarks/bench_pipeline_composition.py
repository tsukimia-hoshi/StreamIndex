...
def _build_inputs(cfg: PipelineCfg):
 ...
 return {
 "q_attn": q_attn, "kv_attn": kv_attn, "q_start": q_start,
 "q_idx": q_idx, "k_idx": k_idx, "weights": weights, "T": T,
 }
...
def _compressed_to_decompressed_indices(top_idx_compressed: torch.Tensor, m: int, S_kv: int):
 ...
 expanded = top_idx_compressed.unsqueeze(-1) * m + torch.arange(m, device=top_idx_compressed.device, dtype=torch.int32)
...
def bench_one(cfg: PipelineCfg, *, indexer: str) -> dict:
 ...
 inputs["q_idx"], inputs["k_idx"], inputs["weights"], cfg.top_k,
 )
 else:
 ...
 cfg.B, cfg.S, S_kv, cfg.H_q, cfg.dim, cfg.tail_dim,
 attn_topk, cfg.kv_stride, kv_group=cfg.H_kv,
 ...
 return kernel(
 inputs["q_attn"], inputs["kv_attn"], attn_idx, inputs["q_start"],
 )
...
def main:
 ...
 print(
 f"{'S':>8}"
 f" {'mat (ms)':>10} {'mat HBM':>10}"
 f" {'chunk (ms)':>11} {'chunk HBM':>11}"
 f" {'mat status':>14}"
 )
 print("-" * 92)
