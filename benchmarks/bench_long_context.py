...
def bench_path_b(cfg: LongContextConfig) -> dict:
 ...
 q, k_idx, weights, top_k=cfg.K,
 chunk_s=cfg.chunk_S, chunk_t=cfg.chunk_T,
 )
...
def main:
 ...
 a_str = (
 f"{r_a['us']/1000:>14.2f}" if r_a["status"] == "ok"
 else f"{_fail_label(r_a['status']):>14}"
 )
 a_hbm = f"{r_a['peak_gb']:>18.2f}" if r_a["status"] == "ok" else f"{'-':>18}"
 b_str = (
 f"{r_b['us']/1000:>14.2f}" if r_b["status"] == "ok"
 else f"{_fail_label(r_b['status']):>14}"
 )
