...
def bench_fwd(cfg: V4Cfg) -> dict:
 ...
 idx = torch.randint(0, cfg.N_kv, (cfg.B, cfg.S, cfg.K),
...
def main:
 ...
 print(f"{'config':<32} {'time (ms)':>10} {'peak (GB)':>10}"
 f" {'TFLOPS':>9} {'% peak':>8}")
 print("-" * 75)
 for cfg in configs:
 r = bench_fwd(cfg)
 ...
 if r["status"] == "ok":
 print(f"{head:<32} {r['us']/1000:>10.2f} {r['peak_gb']:>10.3f}"
 f" {r['tflops']:>9.1f} {r['peak_pct']:>7.1f}%")
 else:
 print(f"{head:<32} {r['status']:>10}"
