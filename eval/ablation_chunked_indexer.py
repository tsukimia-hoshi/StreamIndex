...
def _synthesize_inputs(
    *, B=1, S, n_heads=64, head_dim=128, dim=4096, ratio=4, seed=2026, device="cuda", dtype=torch.bfloat16
):
...
def _ablation_no_merge(q, kv_cache, weights, top_k, ratio, chunk_s, chunk_t):
    ...
    for s_start in range(0, S, chunk_s):
        ...
        chunk_i = torch.where(torch.isinf(chunk_v), torch.full_like(chunk_i, -1), chunk_i)
...
# --------------------------------------------------------------------------
def _ablation_skip_saturated(q, kv_cache, weights, top_k, ratio, chunk_s, chunk_t):
    ...
    for s_start in range(0, S, chunk_s):
        ...
        run_v = torch.full((B, chunk_S, top_k), float("-inf"), dtype=torch.float32, device=q.device)
        run_i = torch.full((B, chunk_S, top_k), -1, dtype=torch.int64, device=q.device)
...
def _ablation_fp16(q, kv_cache, weights, top_k, ratio, chunk_s, chunk_t):
    ...
    for s_start in range(0, S, chunk_s):
        ...
        run_v = torch.full((B, chunk_S, top_k), float("-inf"), dtype=torch.float16, device=q.device)
        run_i = torch.full((B, chunk_S, top_k), -1, dtype=torch.int64, device=q.device)
        ...
        for t_start in range(0, T, chunk_t):
            ...
            run_v, perm = combined_v.topk(top_k, dim=-1)
            run_i = combined_i.gather(-1, perm)
            run_i = torch.where(torch.isinf(run_v.float()), torch.full_like(run_i, -1), run_i)
            out_idx[:, s_start:s_end] = run_i
            return out_idx
...
def main():
    ...
    if not torch.cuda.is_available():
        ...
        print(
            f"Recall test at S={args.small_s} (T={args.small_s // args.ratio}); "
        ...
        idx_prod = _production(q_s, kv_s, w_s, args.top_k, args.ratio, args.chunk_s, args.chunk_t)
        rec = _set_overlap_recall(idx_mat, idx_prod)
        print(
            f"{'production (FP32, full merge)':<32} "
            f"{float(rec.mean()):>9.4f} {float(rec.min):>9.4f} "
            f"{float((rec == 1.0).float().mean()) * 100:>6.2f}%"
        )

        # A1 — no merge (first-chunk only)
        idx_a1 = _ablation_no_merge(q_s, kv_s, w_s, args.top_k, args.ratio, args.chunk_s, args.chunk_t)
        rec = _set_overlap_recall(idx_mat, idx_a1)
        print(
            f"{'A1: no per-chunk merge':<32} "
            f"{float(rec.mean()):>9.4f} {float(rec.min):>9.4f} "
            f"{float((rec == 1.0).float().mean()) * 100:>6.2f}%"
        )

        # A2 — skip saturated chunks. Use chunk_t < top_k to trigger saturation.
        idx_a2 = _ablation_skip_saturated(
            q_s, kv_s, w_s, args.top_k, args.ratio, args.chunk_s, args.saturated_chunk_t
        )
        rec = _set_overlap_recall(idx_mat, idx_a2)
        print(
            f"{f'A2: skip saturated (ct={args.saturated_chunk_t})':<32} "
            f"{float(rec.mean()):>9.4f} {float(rec.min):>9.4f} "
            f"{float((rec == 1.0).float().mean()) * 100:>6.2f}%"
        )

        # A2-control — production at the same small chunk_t (should give 100%)
        idx_a2_ctrl = _production(
            q_s, kv_s, w_s, args.top_k, args.ratio, args.chunk_s, args.saturated_chunk_t
        )
        rec = _set_overlap_recall(idx_mat, idx_a2_ctrl)
        print(
            f"{f'A2-ctrl: production (ct={args.saturated_chunk_t})':<32} "
            f"{float(rec.mean()):>9.4f} {float(rec.min):>9.4f} "
            f"{float((rec == 1.0).float().mean()) * 100:>6.2f}%"
        )

        # A3 — FP16 accumulation
        idx_a3 = _ablation_fp16(q_s, kv_s, w_s, args.top_k, args.ratio, args.chunk_s, args.chunk_t)
        rec = _set_overlap_recall(idx_mat, idx_a3)
        print(
            f"{'A3: FP16 score accumulation':<32} "
            f"{float(rec.mean()):>9.4f} {float(rec.min):>9.4f} "
            f"{float((rec == 1.0).float().mean()) * 100:>6.2f}%"
        )
        ...
        fn_prod = lambda: _production(q_b, kv_b, w_b, args.top_k, args.ratio, args.chunk_s, args.chunk_t)
        peak_prod = _measure_peak(fn_prod)
        us_prod = _time_us(fn_prod)
        print(f"{'production (FP32)':<32} {us_prod / 1000:>10.1f} {peak_prod:>9.2f} GB")

        fn_a3 = lambda: _ablation_fp16(q_b, kv_b, w_b, args.top_k, args.ratio, args.chunk_s, args.chunk_t)
        peak_a3 = _measure_peak(fn_a3)
        us_a3 = _time_us(fn_a3)
        print(f"{'A3: FP16':<32} {us_a3 / 1000:>10.1f} {peak_a3:>9.2f} GB")
        print(
            f"{' (speedup vs prod)':<32} {us_prod / us_a3:>10.2f}x {(peak_prod / peak_a3 if peak_a3 else 0):>7.2f}x lower peak"
        )
