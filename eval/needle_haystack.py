...
def _build_one(
    ...
    """
    needle_text = f"\n\nThe magic number for this document is {needle_value}. Remember it.\n\n"
    ...
    for _ in range(20):
        ...
        while cut < len(passage) and passage[cut] != " ":
            cut += 1
            prompt = instruction + passage[:cut] + needle_text + passage[cut:] + question
...
def main():
    ...
    p.add_argument("--max-seq-len", type=int, required=True, help="Target prompt length in tokens.")
    p.add_argument(
        "--n-samples", type=int, default=50, help="Number of prompts to generate (split across depths)."
    )
    p.add_argument(
        "--depths",
        type=str,
    ...
    p.add_argument("--tokenizer", type=str, required=True, help="HF tokenizer name or path.")
...
def score(samples_path: str, predictions_path: str) -> dict:
    ...
    preds = {rec["id"]: rec["prediction"] for rec in (json.loads(l) for l in open(predictions_path))}
    by_depth = {}
    correct = 0
    n = 0
