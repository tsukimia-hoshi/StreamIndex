...
def _chunked_topk(q, kv_cache, weights, top_k: int, ratio: int, chunk_s: int, chunk_t: int):
...
def bench_one(S: int, indexer: str, *, n_heads, head_dim, dim, top_k, ratio, chunk_s, chunk_t):
    ...
    try:
        ...
        if indexer == "materialize":
            ...
            fn = lambda: _chunked_topk(q, kv_cache, weights, top_k, ratio, chunk_s=chunk_s, chunk_t=chunk_t)
        else:
...
def main():
    ...
    ap.add_argument(
        "--seq-lens", default="32768,65536,131072,262144,524288,1048576", help="Comma-separated S values."
    )
