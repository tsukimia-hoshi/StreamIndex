 ...
 *, B=1, S, n_heads=64, head_dim=128, dim=4096, ratio=4,
 seed=2026, device="cuda", dtype=torch.bfloat16,
):
...
# --------------------------------------------------------------------------
def _run_one(S, *, chunk_s, chunk_t, top_k, n_heads, head_dim, dim, ratio,
 measure_recall: bool):
 try:
...
def main:
 ...
 help="S where only chunked runs (cost measurement).")
 ...
 r_small = _run_one(args.small_s, chunk_s=chunk_s, chunk_t=8192,
 top_k=512, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=True)
 torch.cuda.empty_cache
 r_big = _run_one(args.big_s, chunk_s=chunk_s, chunk_t=8192,
 top_k=512, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=False)
 torch.cuda.empty_cache
 rec = (f"{r_small['recall'][0]:.4f}", f"{r_small['recall'][1]:.4f}") \
 if r_small["status"] == "ok" else ("-", "-")
 sm_t = (f"{r_small['us']/1000:.1f}", f"{r_small['peak_gb']:.2f}GB") \
 if r_small["status"] == "ok" else (r_small["status"], "-")
 bg_t = (f"{r_big['us']/1000:.1f}", f"{r_big['peak_gb']:.2f}GB") \
 if r_big["status"] == "ok" else (r_big["status"], "-")
 print(f"{chunk_s:>10} {rec[0]:>9} {rec[1]:>9}"
 ...
 r_small = _run_one(args.small_s, chunk_s=2048, chunk_t=chunk_t,
 top_k=512, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=True)
 torch.cuda.empty_cache
 r_big = _run_one(args.big_s, chunk_s=2048, chunk_t=chunk_t,
 top_k=512, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=False)
 torch.cuda.empty_cache
 ...
 bg_t = (f"{r_big['us']/1000:.1f}", f"{r_big['peak_gb']:.2f}GB") \
 if r_big["status"] == "ok" else (r_big["status"], "-")
 print(f"{chunk_t:>10} {rec[0]:>9} {rec[1]:>9}"
 f" {sm_t[0]:>9} {sm_t[1]:>10}"
 f" {bg_t[0]:>9} {bg_t[1]:>10}")
 ...
 r_small = _run_one(args.small_s, chunk_s=2048, chunk_t=8192,
 top_k=top_k, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=True)
 torch.cuda.empty_cache
 r_big = _run_one(args.big_s, chunk_s=2048, chunk_t=8192,
 top_k=top_k, n_heads=args.n_heads,
 head_dim=args.head_dim, dim=args.dim, ratio=args.ratio,
 measure_recall=False)
 torch.cuda.empty_cache
 ...
 bg_t = (f"{r_big['us']/1000:.1f}", f"{r_big['peak_gb']:.2f}GB") \
 if r_big["status"] == "ok" else (r_big["status"], "-")
 print(f"{top_k:>10} {rec[0]:>9} {rec[1]:>9}"
 f" {sm_t[0]:>9} {sm_t[1]:>10}"
 f" {bg_t[0]:>9} {bg_t[1]:>10}")
