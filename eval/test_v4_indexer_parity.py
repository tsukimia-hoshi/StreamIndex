...
def _chunked_topk(q, kv_cache, weights, top_k: int, ratio: int, chunk_s: int = 2048, chunk_t: int = 8192):
...
def main():
    ...
    ap.add_argument("--seq-lens", default="2048,4096,8192", help="Comma-separated S values to test.")
    ...
    if not torch.cuda.is_available():
        ...
        for S in seq_lens:
            ...
            passed = (mean_r >= args.threshold) and ((not args.require_perfect_min) or (min_r >= 1.0 - 1e-6))
            verdict = "PASS" if passed else "FAIL"
            if not passed:
                overall_pass = False
                print(
                    f"{S:>8} {T:>8} {mean_r:>8.4f} {min_r:>8.4f} "
