...
def main:
 ...
 print(f"required libs missing: {e}; pip install torch transformers",
 file=sys.stderr)
 sys.exit(1)
 ...
 print(f"Patched {n_patched} attention layers to use chunked indexer",
 file=sys.stderr)
 else:
 ...
 print(f"[{n_done}] {rec['id']} {metrics.get('status')}: "
 f"{metrics.get('msg', '')[:80]}", file=sys.stderr)
